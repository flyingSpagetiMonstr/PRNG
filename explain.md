...
### 关键
- `f[]`为一个用完即弃（因为每轮更新）的映射，且`f[]`完全伪随机后还具有非线性、不可逆（非满射）的性质（`f[]`在更新过程中经过线性或满射状态的频率极低）
- 若`f[]`未被赋种，则由有序的初态到完全伪随机状态的期望轮数为更新255个元素的所需轮数，即略小于 $E[256]$ ，到达该轮数之前的输出可以考虑舍弃。

#### 状态无法反推
1. 由于`update()`中`a`、`b`、`c`的迭代多次用到`f[]`，并用该结果更新`f[]`，故由当前状态无法倒推之前状态，更无法获得之前的输出。

2. 此外，上述结果还用于更新整个`f[]`，故由当前状态无法获知之前任何元素的值，除非猜，但这里猜的效率很低，具体可以参考代码稍作分析。

#### 能否通过Next-bit test
设计时，由于该程序每次的输出均为`f[]`的某个元素，会暴露内部状态，导致输出更容易预测，故每轮更新过程中增加了[对`f[i]`以外元素的修改](#phi中bc的去处)。

增加此修改后，针对Next-bit test分析如下：
1. 该算法输出0和1的数量相等，故在不知道算法结构的情况下无法预测
2. 考虑算法结构，则有：
   1. 易知`i`为完全暴露，分析中亦不考虑`x`的作用，认为`x`也被知晓
   2. 故预测输出即为预测`update()`后`f[]`在`i`处的值
   3. 而`f[]`在`update()`中被更改，欲获知`f[i]`，必须获取`f[]`中若干个（>1个，具体参见代码）其它元素的值
   4. 每次输出只会暴露`f[]`中一个元素的值，接下来证明任意一轮暴露的值（攻击者获取的信息）无法向前（更新前）或向后（下次更新）积累，即无法获取任一状态下`f[]`中的多个值

3. 无法向更新前积累，即 $f[a]_{current}$ 被知晓，不会泄露 $f[a]_{past}$ ，这是[状态无法反推](#状态无法反推)中第二点的子结论
4. 无法向下次更新积累，即 $f[a]_{current}$ 被知晓，不会泄露 $f[a]_{subsequent}$ ，程序中 $f[a]$ 被输出后即丢弃并更新，故不会透露 $f[a]_{subsequent}$。
